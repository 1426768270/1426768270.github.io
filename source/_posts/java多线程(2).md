...
java6之前的synchronized属于**重量锁**,性能较差,
...
![synchronized底层原理1](java多线程(2)/synchronized底层原理1.png)

使用 javap -c -v -l 指令反编译 class文件后的 **字节码指令** 如下

![synchronized底层原理2](java多线程(2)/synchronized底层原理2.png)
...
![synchronized底层原理3](java多线程(2)/synchronized底层原理3.png) 
...
![synchronized底层原理4](java多线程(2)/synchronized底层原理4.png)
...
sync(Object) 系统中记录这个ID （偏向锁）

如果线程征用升级为自旋锁，10次以后升级为重量级锁 进入OS的等待队列
...
子类的同步方法上调用父类的同步方法，锁的对象还是子类this



#### 加锁时抛出异常

加锁时抛出异常会让锁释放，让其他想获得锁的对象获得
...
- Synchronized和Reentrantlock都属于**可重入锁**。
...
#### 公平锁
公平锁指根据线程在**队列**中的**优先级**获取锁,比如线程优先加入阻塞队列,那么线程就优先获取锁。
...
用对象作为锁的时候 应该加上final 保证锁的对象不变

### volatile
volatile是JVM提供的轻量级的线程同步机制。它可以保证内存的可见性，禁止指令重排序。
但是volatile，并**不能保证数据的原子性**，所以它不合适作为线程同步的工具。
...
![Java内存模型](java多线程(2)/Java内存模型.png)
...
然后写回主内存，其它内存才知道i的值被修改了，这个**过程本身就不是原子**的。
...
这个类较普通的原子类新增了一个**stamp**字段，它的作用相当于version。
...
static AtomicStampedReference<Integer> atomicStampedReference = new AtomicStampedReference<>(1,1);//后面的1就是时间戳
```
...
* 直接操作类或实例变量
  ...
  * objectFieldOffset
* CAS相关操作
  * weekCompareAndSetObject Int Long

#### LongAdder

在线程多，循环次数多的情况下 比CAS好
