...
但是缺点更明显:1.每次只使用内存的一半，就代表可使用的内存减少了1/2，代价很高昂。2.如果全部存活，要全部复制一遍，对象的存活率必须低**
